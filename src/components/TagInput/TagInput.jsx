...
const TagInput = ({ required = false, min = 0, max = 99999, onChange = null, suggestions = [], strict = false }) => {
  ...
  const handleChangeInputValue = (e) => {
    ...
    const lastKey = e.target.value[e.target.value.length - 1]
    const targetValue = e.target.value

    // verifica si el ultimo caracter no está dentro de los delimitadores para actualizar el estado
    if (!disabled.includes(lastKey)) {
      setInput(targetValue)
    }

    if (strict) {
      [...suggestedList].some(sug => sug.includes(targetValue)) || targetValue === ''
        ? setAlert('')
  ...
  const handleKeyCodeActions = (e) => {
    ...
    if (e.target.value.length === 0 && e.key === 'Backspace' && tags.length > 0) {
      ...
      const inputValue = handleFormatText(input)
      if (inputValue.length > 0 && !disabled.includes(inputValue)) {
        // eslint-disable-next-line no-unused-expressions
        strict
          ? [...suggestedList].includes(inputValue) ? (handleAddTag(inputValue), setAlert('')) : setAlert('border border-danger')
  ...
  const handleAddTag = (value) => {
    const listTags = [...tags]
    if (!listTags.includes(value) && listTags.length < max) {
    ...
    <div className={`form-control d-flex bd-highlight flex-wrap ${alert}`}>

      {
        [...tags].map((tag, key) =>
          <span
            key={'tag-' + key}
            className="badge bg-primary me-1 mb-1 text-break"
          >{tag}</span>
        )
      }
